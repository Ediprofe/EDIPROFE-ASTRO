# **Propiedades de la materia**

Las **propiedades de la materia** se dividen en dos grandes grupos: **Cualitativas** y **Cuantitativas**

---

## 🎨 **Propiedades cualitativas**

- Describen **cualidades** del material.
- No incluyen **números ni unidades**.
- Se basan en la **observación sensorial**.

### Ejemplos

| **Propiedad** | **Descripción** |
| :------------ | :-------------- |
| Sabor         | dulce           |
| Color         | blanco          |
| Textura       | cremosa         |

---

## 📏 **Propiedades cuantitativas**

- Describen **cantidades medibles**.
- Incluyen **valor numérico y unidad**.
- Se obtienen mediante **instrumentos de medición**.

### Ejemplos

| **Propiedad** | **Valor** |      **Unidad**     |
| :------------ | :-------: | :-----------------: |
| Masa          |    2300   |     $\mathrm{g}$    |
| Volumen       |    1.5    |     $\mathrm{L}$    |
| Densidad      |     1     |    $\mathrm{g/L}$   |
| Temperatura   |    36.3   | $\mathrm{^\circ C}$ |
| Conductividad |    500    | $\mathrm{\mu S/cm}$ |
